...
nb_epoch = 20
nb_features = 1
model = autoencoder_fully_convolutional(window_size,nb_features)
result_files = use_whole_data(data_files,window_size,nb_features,train_autoencoder_based_models,model)
with open('autoencoder_fully_convolutional.obj','wb') as f:
    pickle.dump(result_files,f)
write_result(algorithm_name='autoencoder_fully_convolutional',data_files=result_files,results_path=cwd+'/results')