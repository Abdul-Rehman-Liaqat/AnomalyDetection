...
def train_nStepPrediction_based_models_new(df,
    ...
    print(df.head(),"22222222222")
    df['convergenceLoss'] = addDummyData(convergence_loss,length)
    print(df.head(),"333333333333")
    df['anomaly_score'] = df[anomaly_score]
    print(df.head(),"444444444444")
    return df
...
def train_autoencoder_based_models_new(df,
                                       model,
                                       input_shape,
                                       nb_epoch=1,
                                       anomaly_score = "convergenceLoss",
    ...
    df['convergenceLoss'] = [convergence_loss[0]]*(input_shape[0]) +\
    convergence_loss
    df['anomaly_score'] = df[anomaly_score]
    return df
