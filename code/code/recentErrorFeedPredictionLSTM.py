...
import os
from models import predictionLstmStepAheadWithErrorFeed
...
def train_nStepPrediction_based_models_new(df,
    ...
    for ind,i in enumerate(np.arange(start_point,len(df))):
        ...
        X_input = X_input.reshape((1,)+(input_shape[0]+errorInput,input_shape[1]))
...
for file in all_files_path:
    if(not ".md" in file):
        df = pd.read_csv(file)
        model = predictionLstmStepAheadWithErrorFeed(input_shape,multistep)
