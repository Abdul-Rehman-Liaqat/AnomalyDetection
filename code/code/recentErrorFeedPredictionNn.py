...
import os
from models import predictionNnStepAheadWithErrorFeed
...
def train_nStepPrediction_based_models_new(df,
    ...
    maxVal = 0
    W = input_shape[0]
    errorInputArray = [0] * errorInput
    start_point = W+nStepAhead
    for ind,i in enumerate(np.arange(start_point,len(df))):
        X_input = df["value"].values[i - (start_point):\
                    i-nStepAhead]
        X_input = errorInputArray + X_input.tolist()
        if(len(convergence_loss) >= errorInput):
            X_input[0:errorInput] = convergence_loss[-errorInput:]
        else:
            X_input[errorInput-len(convergence_loss):errorInput] = convergence_loss[:] 
        X_input = np.array(X_input)
        X_input = X_input.reshape((1,)+(input_shape[0]+errorInput,))
        Y_input = df["value"].values[i-nStepAhead:i]
        Y_input = Y_input.reshape((1,)+(nStepAhead,))
        pred = (model.predict(X_input))
        prediction.append(pred[0].tolist())
...
algo_core = "recentErrorpredictionMultiStep"
algo_type = "NN"
...
str(window_size)+"errorInput"+str(errorInput)+anomalyScore_func+anomalyScore_type+add_to_name
...
for file in all_files_path:
    if(not ".md" in file):
        df = pd.read_csv(file)
        model = predictionNnStepAheadWithErrorFeed(input_shape,multistep,errorInput = errorInput)
